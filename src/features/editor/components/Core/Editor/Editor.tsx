...
import { SlashMenu } from "@/features/SlashMenu/extensions/SlashMenu";
...
export default function Editor() {
  ...
  const isMathEnabled =
    plugins.find((p) => p.id === "latex-math")?.status === "enabled";
  ...
  const getRelativeSubfolder = () => {
    ...
    const folderPath = activeFile.substring(
      0,
      activeFile.lastIndexOf(separator),
    );
      ...
      onUpdate: ({ editor }) => {
        ...
        if (currentPath) {
          saveTimeoutRef.current = setTimeout(
            () => save(currentPath, rootPath || undefined),
            1500,
          );
        }
      },
    ...
    const handleMouseUp = (e: MouseEvent) => {
      ...
      if (sourceNodePos !== null) {
        ...
        if (
          ...
          const isPdf = PDF_EXTENSIONS.some((ext) =>
            sourcePath.toLowerCase().endsWith(ext),
          );
  ...
  return (
    ...
    >
      ...
      {showHistory && (
        <VersionHistory editor={editor} onClose={() => setShowHistory(false)} />
      )}
      <ConfirmModal
