...
import {
  ...
  Edit2, Trash2, Layers, MoreHorizontal, ChevronRight, X
} from 'lucide-react';
...
export default function AssetGallery({
  ...
  const allItems = useMemo(() => {
    const items: GalleryItem[] = [
      ...filteredCollections.map(col => ({ type: 'collection' as const, data: col })),
  ...
  const renderGalleryItem = (item: GalleryItem, _style: React.CSSProperties, index: number) => {
    if (item.type === 'collection') {
      const col = item.data;
      return (
        <div 
        ...
        >
          {!disableOrganization && <button className={styles.folderCard__delete} onClick={(e) => { e.stopPropagation(); setCollectionToDelete(col.id); }}><Trash2 size={14} /></button>}
          <div className={styles.folderCard__previews}>
            {col.images.length > 0 ? (
              <div className={styles.folderCard__heroLayout}>
                <div className={styles.folderCard__hero}><LazyImage src={resolveAssetPath(col.images[0], rootPath) || ""} alt="" /></div>
                <div className={styles.folderCard__sidebar}>
                  {col.images.slice(1, 4).map((p: string, i: number) => <div key={i} className={styles.folderCard__sideItem}><LazyImage src={resolveAssetPath(p, rootPath) || ""} alt="" /></div>)}
          ...
          <div className={styles.folderCard__glassInfo}>
            <div className={styles.folderCard__text} onClick={(e) => !disableOrganization && handleOpenRename(col.id, e)}>
    ...
    return (
      <div 
        ...
        onMouseDown={(e) => !disableOrganization && handleMouseDown(e, img)} 
        ...
        onClick={() => { 
          ...
          if (isSelectionMode) { 
            setSelectedPaths(prev => prev.includes(img.path) ? prev.filter(p => p !== img.path) : [...prev, img.path]); 
      ...
      >
        <div className={styles.card__preview}>
          <LazyImage src={resolveAssetPath(img.path, rootPath) || ""} alt={img.name} />
        </div>
  ...
  return (
    <div className={`${styles.container} ${pickerMode ? styles['container--picker'] : ''}`}>
      <header className={styles.header}>
        ...
        <div className={styles.actions}>
          ...
          {onClose && (
            <button className={styles.closeBtn} onClick={onClose} title="Fechar Galeria">
              <X size={20} />
            </button>
          )}
        </div>
      ...
      <div className={styles.content}>
        <VirtualMasonry
          ...
          columnWidth={320}
          gap={20}
          getItemKey={(item) => item.type === 'collection' ? item.data.id : item.data.full_path}
